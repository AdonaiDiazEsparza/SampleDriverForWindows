...
#define DRIVER_NAME_DEBUG "EVENT_CATCHER: "
...
extern "C" 
NTSATUS DriverEntry(PDRIVER_OBJECT  DriverObject,PUNICODE_STRING RegistryPath)
{
    ...
    KdPrint((DRIVER_NAME_DEBUG "======> DRIVER ENTRY POINT\n"));
    ...
    if(!NT_SUCCESS(status)){
        KdPrint((DRIVER_NAME_DEBUG " Error Creating object: 0x%x\n", status));
        return status;
    }

    /* Set Entry Points */
    deviceObject->DriverUnload = DriverUnloadRoutine;
    ...
    if (!NT_SUCCESS(status)) {
        ...
        KdPrint((DRIVER_NAME_DEBUG "IoCreateSymbolicLink returned 0x%x\n", status));
        return(status);
    }
...
{
    KdPrint((DRIVER_NAME_DEBUG "====>Unloading driver"));
